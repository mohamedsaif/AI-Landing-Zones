...
@description('Required. When true, the deployment will pause for a fixed, predetermined number of seconds before continuing; set to false to skip the wait.')
param waitForConnections bool

@description('Required. When true, the deployment will pause for a fixed, predetermined number of seconds before proceeding; set to false to continue immediately without waiting.')
param waitForProject bool
...
module waitForProjectScript 'waitDeploymentScript.bicep' = if (waitForProject) {
...
module cosmosDbRoleAssignments 'role-assignments/cosmosDb.bicep' = if (!empty(cosmosDbConnection)) {
  ...
  scope: resourceGroup(cosmosDbConnection!.subscriptionId, cosmosDbConnection!.resourceGroupName)
  dependsOn: [
    ...(waitForProject ? [waitForProjectScript] : [])
  ]
  params: {
...
module waitForConnectionsScript 'waitDeploymentScript.bicep' = if (waitForConnections) {
...
resource accountCapabilityHost 'Microsoft.CognitiveServices/accounts/capabilityHosts@2025-04-01-preview' = if (createAccountCapabilityHostInternal) {
  ...
  dependsOn: [
    project
    ...(waitForProject ? [waitForConnectionsScript] : [])
    cosmosDbConnectionResource
    storageAccountConnectionResource
    aiSearchConnectionResource
  ]
